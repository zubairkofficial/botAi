...
class BlogsController extends Controller
{
    ...
    public function store(BlogRequestForm $request)
    {
        ...
        if ($request->slug != null) {
            $blog->slug = preg_replace('/\s+/', '-', trim($request->slug));
        } else {
            $blog->slug = preg_replace('/\s+/', '-', trim($request->title)) . '-' . Str::random(5);
        }
    ...
    public function update(BlogRequestForm $request)
    {
        ...
        if ($request->lang_key == env("DEFAULT_LANGUAGE")) {
            $blog->title = $request->title;
            $blog->slug = (!is_null($request->slug)) ? preg_replace('/\s+/', '-', trim($request->slug)) : preg_replace('/\s+/', '-', trim($request->name)) . '-' . strtolower(Str::random(5));
            $blog->blog_category_id = $request->category_id;
